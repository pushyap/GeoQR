...
// =====================================
// In-memory store for attendance OTP
// =====================================
const attendanceOtpStore = new Map();
...
router.post('/send-otp', authenticate, async (req, res) => {
    try {
        ...
        // Store with 2 min expiry
        attendanceOtpStore.set(userId, {
            otp,
            expiresAt: Date.now() + 2 * 60 * 1000
        });

        // Send email
        const emailResult = await sendEmail(email, 'loginOtp', name, otp); // Reusing loginOtp template is fine

        if (emailResult.success) {
            res.json({ success: true, message: 'OTP sent to your email' });
        } else {
            console.error('Send attendance OTP failed:', emailResult.error);
            res.status(500).json({ success: false, error: 'Failed to send OTP email' });
        }
    } catch (err) {
        console.error('Send attendance OTP error:', err);
        res.status(500).json({ success: false, error: 'Server error' });
    }
});
...
// Verify attendance OTP and return token
// =====================================
router.post('/verify-otp', authenticate, [
    ...
    const record = attendanceOtpStore.get(userId);
    ...
    if (Date.now() > record.expiresAt) {
        attendanceOtpStore.delete(userId);
        return res.status(400).json({ success: false, error: 'OTP expired' });
    }
    ...
    // Generate short-lived OTP token for attendance scan
    const otpToken = crypto.randomBytes(32).toString('hex');

    try {
        await db.query(
            `INSERT INTO webauthn_challenges (user_id, challenge, type, expires_at) 
             VALUES ($1, $2, 'otp_verification', NOW() + INTERVAL '5 minutes')`,
            [userId, otpToken]
        );
        ...
        console.error('Verify attendance OTP error:', err);
        res.status(500).json({ success: false, error: 'Server error' });
    }
});
